...
public class LinkServiceTest {
    ...
    @Test
    public void createShortLinkShouldReturnLinkFromDb() {
        entityManager.persist(new Link("/l/" + LINK, CORRECT_URL));
        LinkService linkService = new LinkService(linkRepository);
        Map<String, String> originalLinkMap = new HashMap<>();
        originalLinkMap.put("original", CORRECT_URL);
        assertThat(linkService.createShortLink(originalLinkMap).get("link")).isEqualTo("/l/" + LINK);
    }
    ...
    public void getOriginalByShortUrlShouldGetAndIncrementCounter() throws URISyntaxException {
        LinkService linkService = new LinkService(linkRepository);
        int count = 0;
        Link link = new Link("/l/" + LINK, CORRECT_URL, 0, count);
